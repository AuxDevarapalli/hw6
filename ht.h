...
public:
    ...
    HASH_INDEX_T next() 
    {
      ...
      //this->numProbes_++;
      HASH_INDEX_T num= ((this->dhstep_*this->numProbes_)+this->start_)%this->m_;
      this->numProbes_++;
      return num;
       ...
       :  hash_(hash), kequal_(kequal), prober_(prober)
{
    ...
    mIndex_=0;
    //std::vector<HashItem*> table_[CAPACITIES[mIndex_]]
...
{
  //need to find the number of spots occupied
  //this loop is causing the 100 s run cuz it runs like a million times at o(m)
  //double n=0.0;
  // for(int i=0; i<table_.size();i++){
  //   if(table_[i]!=nullptr){
  //     n+=1.0;
  //   }
  // }
  //first check the size to make sure we can procee
  if((fill_/table_.size())>=sizer_){
    //must resize of there is no space
    this->resize();
  }
  //save probe to check 
  HASH_INDEX_T val=probe(p.first);
  //throw eerrro
  if(val==npos){
    throw std::logic_error("error not found");
  }
  //replcae
  else if(table_[val]!=nullptr){
    table_[val]->item.second=p.second;
  }
  //empty
  else if(table_[val]==nullptr){
    table_[val]=new HashItem(p);
    fill_+=1.0;
  }
...
{
  HASH_INDEX_T val=probe(key);
  // if(table_[val]==key){
  //   table_[val]->deleted=true;
  // }
  //make sure not npos
  if(val==npos){
  ...
  //null meaning nothing to mark delete
  if(table_[val]==nullptr){
    return;
  }
  //use kequal and then mark it
  if(kequal_(key,table_[val]->item.first)){
    table_[val]->deleted=true;
    return;
  }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  //fill_=0;
  //if no space then throw erro since 28 is max
  // if(27<=mIndex_){
  // 
  //reset fill
  fill_=0;
  mIndex_++;
  std::vector<HashItem*> newV(CAPACITIES[mIndex_],nullptr);
  // need to run through full talbe and all vals
  for(int i=0; i<table_.size();i++){
    //if empty then dont do anything
    if(table_[i]==nullptr){
      //do nothing
      //newV[hash]=table_[i];
    }
    //if theres something there, must od something
    else if(table_[i]!=nullptr){
      //this is when it is not delted
      if(table_[i]->deleted==false){
        //newV.insert(table_[i]->item);
        //get ket
        KeyType k=table_[i]->item.first;
        //hash it
        size_t hash=hash_(k)%(CAPACITIES[mIndex_]);
        //using this for the while loop for probing
        bool con=false;
        //if space is empty, go ahead adn insert there
        if(newV[hash]==nullptr){
          newV[hash]=table_[i];
          fill_+=1.0;
        }
        //if space not empty and theres not something (value) ther ein the way
        if(newV[hash]!=table_[i]&& newV[hash]!=nullptr){
          con=true;
          //initate probing
          prober_.init(hash,CAPACITIES[mIndex_],k);
        }
        while(con){
          size_t p=prober_.next();
          //check again if we need to prob or not
          if(newV[p]==nullptr){
            newV[p]=table_[i];
            fill_+=1.0;
            con=false;
      ...
      //if it was marked as delete,  delete
